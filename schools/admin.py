...
class PartnerSchoolAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('🏫 School Identity', {
            'fields': ('admin', 'name', 'school_type', 'motto', 'logo', 'image'),
            'classes': ('wide',)
        }),
        ...
        ('📚 Academic Information', {
            'fields': ('combinations_offered', 'a_level_students', 'o_level_streams',
                       'year_founded', 'facilities', 'boarding', 'description'),
            'classes': ('wide',)
        }),
        ('💰 Advertising & Visibility', {
            'fields': ('ad_tier', 'is_partner', 'is_featured', 'is_active'),
    ...
    def school_icon(self, obj):
        """Display school icon with logo if available – SAFE version"""
    ...
    # Bulk actions – kept but safe
    actions = [
        'make_partner', 'remove_partner', 'make_featured', 'remove_featured',
        'activate_schools', 'deactivate_schools',
        'set_premium_tier', 'set_standard_tier', 'set_free_tier'
    ]
    ...
    def save_model(self, request, obj, form, change):
        if not obj.admin:
            ...
            base_email = f"{obj.name.lower().replace(' ', '.')}@school.ug"
